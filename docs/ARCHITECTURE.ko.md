...
있습니다. `cfg.local_heuristic`이 참(`HERMES_LOCAL_HEURISTIC=1`)이면 이 검사
자체를 건너뜁니다 — 이 모드는 자격 증명을 검증할 네트워크 클라이언트가
아예 없습니다.

`LLMClient`(`processor/llm/client.py`)는 `__init__`에서 `cfg.local_heuristic`에
따라 분기합니다: `cfg.api_url` 대상 `openai.OpenAI` 클라이언트이거나,
`LocalHeuristicEngine`(`processor/llm/local_engine.py`) — 네트워크 호출 없이
TF-IDF+사전 매칭으로 대체하는 결정론적 엔진. 둘 다 `ask()` 메서드 하나를
공유해서 `SummaryProcessor`, `EntityProcessor`, `KeywordProcessor`,
`RelatedProcessor`, `DescriptionFillProcessor`는 어느 백엔드가 활성인지 몰라도
됩니다. 응답 캐시 키는 백엔드+모델별로 네임스페이스가 나뉘어있어서
(`local-heuristic-v1` vs `remote:<model>`) `HERMES_LOCAL_HEURISTIC`을 전환해도
다른 백엔드가 생성한 캐시 답변이 재생되지 않습니다.
