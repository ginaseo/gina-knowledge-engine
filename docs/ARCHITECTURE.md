...
so non-LLM processors can run without credentials. Skipped entirely when
`cfg.local_heuristic` is true (`HERMES_LOCAL_HEURISTIC=1`) — that mode has no
network client to validate credentials for.

`LLMClient` (`processor/llm/client.py`) branches on `cfg.local_heuristic` in
`__init__`: either an `openai.OpenAI` client against `cfg.api_url`, or a
`LocalHeuristicEngine` (`processor/llm/local_engine.py`) — a deterministic
TF-IDF + dictionary-matching stand-in that never calls the network. Both paths
share one `ask()` method, so `SummaryProcessor`, `EntityProcessor`,
`KeywordProcessor`, `RelatedProcessor`, and `DescriptionFillProcessor` don't
need to know which backend is active. Response cache keys are namespaced by
backend and model (`local-heuristic-v1` vs. `remote:<model>`) so switching
`HERMES_LOCAL_HEURISTIC` doesn't replay a cached answer generated by the other
backend.
